...
struct TrackerOut: Module {
	...
	void	process(const ProcessArgs& args) override {
		...
		for (i = 0; i < synth->channel_count; ++i) {
			this->outputs[OUTPUT_PITCH].setVoltage(synth->out_synth[i * 3 + 0], i);
			this->outputs[OUTPUT_GATE].setVoltage(synth->out_synth[i * 3 + 1], i);
			this->outputs[OUTPUT_VELOCITY].setVoltage(synth->out_synth[i * 3 + 2], i);
		}
		/// SET OUTPUT CV
		for (i = 0; i < 8; ++i) {
...
struct TrackerOutWidget : ModuleWidget {
	...
	TrackerOutWidget(TrackerOut* _module) {
		...
		addOutput(
		/**/ createOutputCentered<PJ301MPort>(mm2px(Vec(9.0, 55.0)),
		/**/ module,
		/**/ TrackerOut::OUTPUT_PITCH));
		addOutput(
		/**/ createOutputCentered<PJ301MPort>(mm2px(Vec(9.0 + 12.0, 55.0)),
		/**/ module,
		/**/ TrackerOut::OUTPUT_GATE));
		addOutput(
		/**/ createOutputCentered<PJ301MPort>(mm2px(Vec(9.0 + 12.0 * 2.0, 55.0)),
		/**/ module,
