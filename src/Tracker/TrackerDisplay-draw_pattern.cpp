...
//void TrackerDisplay::draw_pattern_visual(const DrawArgs &args, Rect rect) {
		...
		if (cv_col->lines[line].mode == PATTERN_CV_SET) {
			...
			if (first_point) {
				...
				if (cv_from != NULL) {
					...
					nvgLineTo(args.vg,
					/**/ sx + (cv_from->value / 1000.0) * CHAR_W * 7.0,
					/**/ sy + CHAR_H * (line_from - 1 - g_editor->pattern_cam_y)
					/**/ + (cv_from->delay / 99.0) * CHAR_H);
				/// WITHOUT PREVIOUS CV
				} else {
					...
					nvgLineTo(args.vg,
					/**/ sx + (cv_to->value / 1000.0) * CHAR_W * 7.0,
					/**/ sy);
				}
			}
			/// DRAW CURVE
